...
export class DistPrimaryOrderAddComponent implements OnInit {
    ...
    if (this.add_list.length > 0) {
      ...
      if (existIndex != -1) {
        ...
        this.add_list.push({
          ...
          'net_price': (parseFloat(subTotal) + parseFloat(gstAmount)) * parseFloat(this.data.qty),
  ...
  save_order() {
    ...
    this.service.post_rqst(this.encryptedData, "Order/primaryOrdersAdd").subscribe(result => {
      this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
      if (this.decryptedData['statusCode'] == 200) {
        ...
        this.router.navigate(['/distribution-list/1/Channel%20Partner/distribution-detail/' + this.encrypt_id + '/' + 'Primary Order'])
      } else {
        this.dialog.error(this.decryptedData['statusMsg']);
      }
    })
