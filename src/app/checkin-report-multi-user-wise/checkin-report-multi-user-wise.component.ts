...
@Component({
  selector: 'app-checkin-report-multi-user-wise',
  templateUrl: './checkin-report-multi-user-wise.component.html',
  styleUrls: ['./checkin-report-multi-user-wise.component.scss']
})
export class CheckinReportMultiUserWiseComponent implements OnInit {
  ...
  fetchReport(action: any = {}) {
    ...
    this.encryptedData = this.service.payLoad ? { "filter": action,'start': this.start, 'pagelimit': this.page_limit } : this.cryptoService.encryptData({ "filter": action,'start': this.start, 'pagelimit': this.page_limit });
    this.service.post_rqst(this.encryptedData, 'Reports/userCheckinmultiUser').subscribe((result) => {
  ...
  openBottomSheet(): void {
    this.filter.filterPage = 'checkinReportForMultiUser';
...
DownloadExcel() {
    this.loader = true;
    this.encryptedData = this.service.payLoad ? { 'filter': this.filter,'start': this.start, 'pagelimit': this.page_limit } :
      this.cryptoService.encryptData({ 'filter': this.filter,'start': this.start, 'pagelimit': this.page_limit });
    this.service.post_rqst(this.encryptedData, "DownloadMaster/downloadMultiUserCheckinDetail")
      .subscribe((result) => {
        this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
  ...
  downloadInChunks() {
    ...
    this.service.post_rqst(this.encryptedData, "DownloadMaster/downloadMultiUserCheckinDetail").subscribe((result) => {
      this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
      if (this.decryptedData['statusCode'] == 200) {
