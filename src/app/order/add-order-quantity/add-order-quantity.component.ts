...
@Component({
  selector: 'app-add-order-quantity',
  templateUrl: './add-order-quantity.component.html',
  animations: [slideToTop()]
  
})
export class AddOrderQuantityComponent implements OnInit {
  ...
  get_product_details(id) {
    ...
    this.data.color = '';
    this.loader = true
    this.encryptedData = this.service.payLoad ? { 'product_id': id, 'type': 'order', 'dr_id': this.data.type_name, 'order_type': 'primary' } : this.cryptoService.encryptData({ 'product_id': id, 'type': 'order', 'dr_id': this.data.type_name, 'order_type': 'primary' });
