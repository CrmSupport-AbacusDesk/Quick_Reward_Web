...
@Component({
  ...
  templateUrl: './loyalty-report-influencer-bonus-point.component.html',
  styleUrls: ['./loyalty-report-influencer-bonus-point.component.scss']
})
export class LoyaltyReportInfluencerBonusPointComponent implements OnInit {
  ...
  getSecondaryProductWiseReport(action: any,length) {
    ...
    this.service.post_rqst(this.encryptedData, 'LoyaltyReport/influencer_bonus_points').subscribe((result) => {
  ...
  getproductWiseSecondaryReportExcel() {
    ...
    this.loader = true;
    this.service.post_rqst(this.encryptedData, "LoyaltyReport/excel_influencer_bonus_points").subscribe((result) => {
      this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
