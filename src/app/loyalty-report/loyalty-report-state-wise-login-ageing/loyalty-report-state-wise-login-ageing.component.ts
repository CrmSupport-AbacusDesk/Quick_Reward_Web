...
@Component({
  ...
  templateUrl: './loyalty-report-state-wise-login-ageing.component.html',
  styleUrls: ['./loyalty-report-state-wise-login-ageing.component.scss']
})
export class LoyaltyReportStateWiseLoginAgeingComponent implements OnInit {
...
//     }
    ...
    this.service.post_rqst({'start': this.start, 'pagelimit': this.page_limit,'filter': this.filter }, 'LoyaltyReport/state_wise_login_ageing').subscribe((resp) => {
      console.log(resp)
  ...
  getproductWiseSecondaryReportExcel() {
    this.loader = true;
    this.service.post_rqst({ 'search': this.search }, "LoyaltyReport/excel_state_wise_login_ageing").subscribe((result) => {
      if (result['msg'] == true) {
        this.loader = false;
