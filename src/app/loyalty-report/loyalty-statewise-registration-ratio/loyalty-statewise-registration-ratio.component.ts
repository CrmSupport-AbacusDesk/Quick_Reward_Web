...
@Component({
  selector: 'app-loyalty-statewise-registration-ratio',
  templateUrl: './loyalty-statewise-registration-ratio.component.html',
  styleUrls: ['./loyalty-statewise-registration-ratio.component.scss']
})
export class LoyaltyStatewiseRegistrationRatioComponent implements OnInit {
  ...
  fetchReport(action: any = {}) {
    ...
    this.service.post_rqst(this.encryptedData, 'LoyaltyReport/state_wise_registration_ratio_list').subscribe((result) => {
  ...
  openBottomSheet(): void {
    this.filter.filterPage = 'ScanRatioReport',
  ...
  downloadExcel2() {
    ...
    if (can == false) {
      ...
      this.service.post_rqst(this.encryptedData, "LoyaltyReport/state_wise_registration_ratio_report").subscribe((result) => {
        this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
        if (this.decryptedData['code'] === 1) {
