...
@Component({
  ...
  templateUrl: './loyalty-report-influencer-monthwise-scan.component.html',
  styleUrls: ['./loyalty-report-influencer-monthwise-scan.component.scss']
})
export class LoyaltyReportInfluencerMonthwiseScanComponent implements OnInit {
...
// if (this.filter.date_from) {
    ...
    this.service.post_rqst({ 'start': this.start, 'pagelimit': this.page_limit,'filter': this.filter }, 'LoyaltyReport/monthly_scan_report').subscribe((resp) => {
      console.log(resp)
      if (resp['statusCode'] == 200) {
        this.secondaryProductReportList = resp['result'];
        this.header_list = resp.header;
  ...
  getproductWiseSecondaryReportExcel() {
    this.loader = true;
    this.service.post_rqst({ 'search': this.search }, "LoyaltyReport/excel_monthly_scan_report").subscribe((result) => {
      if (result['msg'] == true) {
        this.loader = false;
