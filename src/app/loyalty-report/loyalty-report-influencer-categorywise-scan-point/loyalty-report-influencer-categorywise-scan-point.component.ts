...
@Component({
  ...
  templateUrl: './loyalty-report-influencer-categorywise-scan-point.component.html',
  styleUrls: ['./loyalty-report-influencer-categorywise-scan-point.component.scss']
})
export class LoyaltyReportInfluencerCategorywiseScanPointComponent implements OnInit {
  ...
  getSecondaryProductWiseReport(action: any,length) {
    ...
    this.service.post_rqst(this.encryptedData, 'LoyaltyReport/point_category_wise_scan_report').subscribe((result) => {
  ...
  getproductWiseSecondaryReportExcel() {
    ...
    this.loader = true;
    this.service.post_rqst(this.encryptedData, "LoyaltyReport/excel_point_category_wise_scan_report").subscribe((result) => {
      this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
