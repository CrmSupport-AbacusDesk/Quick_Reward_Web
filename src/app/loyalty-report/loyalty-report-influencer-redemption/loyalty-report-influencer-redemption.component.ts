...
@Component({
  ...
  templateUrl: './loyalty-report-influencer-redemption.component.html',
  styleUrls: ['./loyalty-report-influencer-redemption.component.scss']
})
export class LoyaltyReportInfluencerRedemptionComponent implements OnInit {
  ...
  getSecondaryProductWiseReport(action: any,length) {
    ...
    this.service.post_rqst(this.encryptedData, 'LoyaltyReport/influencer_redemption_report').subscribe((result) => {
  ...
  getproductWiseSecondaryReportExcel() {
    ...
    this.service.post_rqst(this.encryptedData, "LoyaltyReport/excel_influencer_redemption_report").subscribe((result) => {
      this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
