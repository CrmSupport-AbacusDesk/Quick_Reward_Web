...
@Component({
  selector: 'app-loyalty-productwise-scan-ratio',
  templateUrl: './loyalty-productwise-scan-ratio.component.html',
  styleUrls: ['./loyalty-productwise-scan-ratio.component.scss']
})
export class LoyaltyProductwiseScanRatioComponent implements OnInit {
  ...
  fetchReport(action: any = {}) {
    ...
    this.service.post_rqst(this.encryptedData, 'LoyaltyReport/product_wise_scan_ratio_list').subscribe((result) => {
  ...
  downloadExcel2() {
    ...
    if (can == false) {
      ...
      this.service.post_rqst(this.encryptedData, "LoyaltyReport/product_wise_scan_ratio_report").subscribe((result) => {
        this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
        if (this.decryptedData['code'] === 1) {
