...
@Component({
  ...
  templateUrl: './loyalty-report-point-summary.component.html',
  styleUrls: ['./loyalty-report-point-summary.component.scss']
})
export class LoyaltyReportPointSummaryComponent implements OnInit {
  ...
  getSecondaryProductWiseReport(action: any,length) {
    ...
    this.service.post_rqst(this.encryptedData, 'LoyaltyReport/influencer_points_summary_report').subscribe((result) => {
  ...
  getproductWiseSecondaryReportExcel() {
    ...
    this.service.post_rqst(this.encryptedData, "LoyaltyReport/excel_influencer_points_summary_report").subscribe((result) => {
      this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
