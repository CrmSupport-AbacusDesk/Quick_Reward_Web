...
@Component({
  ...
  templateUrl: './loyalty-report-not-scanned-seven-day.component.html',
  styleUrls: ['./loyalty-report-not-scanned-seven-day.component.scss']
})
export class LoyaltyReportNotScannedSevenDayComponent implements OnInit {
...
//     }
    ...
    this.service.post_rqst({'start': this.start, 'pagelimit': this.page_limit,'filter': this.filter }, 'LoyaltyReport/not_scanned_seven_days_influencer').subscribe((resp) => {
      console.log(resp)
  ...
  getproductWiseSecondaryReportExcel() {
    this.loader = true;
    this.service.post_rqst({ 'search': this.search }, "LoyaltyReport/excel_not_scanned_seven_days_influencer").subscribe((result) => {
      if (result['msg'] == true) {
        this.loader = false;
