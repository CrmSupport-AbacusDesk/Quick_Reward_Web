...
@Component({
  selector: 'app-loyalty-scan-ratio-report',
  templateUrl: './loyalty-scan-ratio-report.component.html',
  styleUrls: ['./loyalty-scan-ratio-report.component.scss']
})
export class LoyaltyScanRatioReportComponent implements OnInit {
  ...
  fetchReport(action: any = {}) {
    ...
    this.service.post_rqst(this.encryptedData, 'LoyaltyReport/ScanReportList').subscribe((result) => {
  ...
  downloadExcel2() {
    ...
    if (can == false) {
      ...
      this.service.post_rqst(this.encryptedData, "LoyaltyReport/scan_ratio_report").subscribe((result) => {
        this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
        if (this.decryptedData['code'] === 1) {
