...
@Component({
  ...
  templateUrl: './loyalty-report-influencer-scan-point.component.html',
  styleUrls: ['./loyalty-report-influencer-scan-point.component.scss']
})
export class LoyaltyReportInfluencerScanPointComponent implements OnInit {
  ...
  getSecondaryProductWiseReport(action: any,length) {
    ...
    this.service.post_rqst(this.encryptedData, 'LoyaltyReport/influencer_scan_report').subscribe((result) => {
  ...
  getproductWiseSecondaryReportExcel() {
    ...
    this.loader = true;
    this.service.post_rqst(this.encryptedData, "LoyaltyReport/excel_influencer_scan_report").subscribe((result) => {
      this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
