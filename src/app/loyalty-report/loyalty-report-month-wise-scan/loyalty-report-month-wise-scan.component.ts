...
@Component({
  ...
  templateUrl: './loyalty-report-month-wise-scan.component.html',
  styleUrls: ['./loyalty-report-month-wise-scan.component.scss']
})
export class LoyaltyReportMonthWiseScanComponent implements OnInit {
  ...
  getSecondaryProductWiseReport(action: any,length) {
    ...
    //   this.filter.date_to = '';
    // }
    this.service.post_rqst({'start': this.start, 'pagelimit': this.page_limit,'filter': this.filter }, 'LoyaltyReport/influencer_month_wise_scan_report').subscribe((resp) => {
      console.log(resp)
  ...
  getproductWiseSecondaryReportExcel() {
    this.loader = true;
    this.service.post_rqst({ 'search': this.search }, "LoyaltyReport/excel_influencer_month_wise_scan_report").subscribe((result) => {
      if (result['msg'] == true) {
        this.loader = false;
