...
@Component({
  selector: 'app-pop-gift-report',
  templateUrl: './pop-gift-report.component.html',
  styleUrls: ['./pop-gift-report.component.scss']
})
export class PopGiftReportComponent implements OnInit {
  ...
  fetchReport(action: any = {}) {
    ...
    this.encryptedData = this.service.payLoad ? {} : this.cryptoService.encryptData({});
    this.service.post_rqst(this.encryptedData, 'Reports/popGiftReport').subscribe((result) => {
  ...
  openBottomSheet(): void {
    ...
    this.bottomSheet._openedBottomSheetRef.afterDismissed().subscribe((data) => {
      if (data != undefined) {
        ...
        this.filter.userId = data.user_id;
        this.filter.userName = data.user_name
        this.length = 0;
        this.fetchReport({ 'date_from': this.filter.date_from, 'date_to': this.filter.date_to, 'user_id': this.filter.userId });
      }
    })
  }
  ...
  downloadExcel2() {
    ...
    if (can == false) { 
      ...
      this.encryptedData = this.service.payLoad ? {} : this.cryptoService.encryptData({});
      this.service.post_rqst(this.encryptedData, "DownloadMaster/downloadPopGiftReport").subscribe((result) => {
        this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
        if (this.decryptedData['code'] === 1) {
