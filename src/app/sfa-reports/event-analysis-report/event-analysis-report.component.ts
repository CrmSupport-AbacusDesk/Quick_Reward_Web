...
@Component({
  selector: 'app-event-analysis-report',
  templateUrl: './event-analysis-report.component.html',
  styleUrls: ['./event-analysis-report.component.scss']
})
export class EventAnalysisReportComponent implements OnInit {
  ...
  fetchReport(action: any = {}) {
    ...
    this.service.post_rqst(this.encryptedData, 'Reports/userEventReport').subscribe((result) => {
  ...
  openBottomSheet(): void {
    ...
    this.bottomSheet._openedBottomSheetRef.afterDismissed().subscribe((data) => {
      if (data != undefined) {
        ...
        this.length = 0;
        // 'date_to': this.filter.date_to,
        this.fetchReport({ 'date': this.filter.date_from });
      }
    })
  }
  ...
  downloadExcel2() {
    ...
    if (can == false) { 
      ...
      // , 'date_to': this.filter.date_to
      this.encryptedData = this.service.payLoad ? {} : this.cryptoService.encryptData({});
      this.service.post_rqst(this.encryptedData, "DownloadMaster/downloadUserEventReport").subscribe((result) => {
        this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
        if (this.decryptedData['code'] === 1) {
