...
@Component({
  selector: 'app-visit-ratio-report',
  templateUrl: './visit-ratio-report.component.html',
  styleUrls: ['./visit-ratio-report.component.scss']
})
export class VisitRatioReportComponent implements OnInit {
  ...
  fetchReport(action: any = {}) {
    ...
    this.service.post_rqst(this.encryptedData, 'Reports/visitRatio').subscribe((result) => {
  ...
  openBottomSheet(): void {
    this.filter.filterPage = 'visitRatioReport'
  ...
  downloadExcel2() {
    ...
    if (can == false) {
      ...
      this.service.post_rqst(this.encryptedData, "DownloadMaster/downloadvisitRatio").subscribe((result) => {
        this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
        if (this.decryptedData['code'] === 1) {
