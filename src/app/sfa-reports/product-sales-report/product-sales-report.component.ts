...
export class ProductSalesReportComponent implements OnInit {
  ...
  fetchReport(action: any = {}) {
    ...
    this.service.post_rqst(this.encryptedData, 'Reports/productSaleReport').subscribe((result) => {
  ...
  openBottomSheet(): void {
    this.filter.filterPage = 'productSalesReport',
  ...
  downloadExcel2() {
    ...
    if (can == false) { 
      ...
      this.service.post_rqst(this.encryptedData, "DownloadMaster/downloadProductSaleReport").subscribe((result) => {
        this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
        if (this.decryptedData['code'] === 1) {
