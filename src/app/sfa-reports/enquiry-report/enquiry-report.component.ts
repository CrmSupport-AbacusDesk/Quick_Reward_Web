...
export class EnquiryReportComponent implements OnInit {
  ...
  fetchReport(action: any = {}) {
    ...
    this.service.post_rqst(this.encryptedData, 'Reports/enquiryReport').subscribe((result) => {
  ...
  openBottomSheet(): void {
    this.filter.filterPage = 'enquiryReport',
  ...
  downloadExcel2() {
    ...
    if (can == false) { 
      ...
      this.service.post_rqst(this.encryptedData, "DownloadMaster/downloadEnquiryReport").subscribe((result) => {
        this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
        if (this.decryptedData['code'] === 1) {
