...
export class AttendanceSummaryReportComponent implements OnInit {
  ...
  fetchReport(action: any = {}) {
    ...
    this.service.post_rqst(this.encryptedData, 'Reports/userAttendanceMonthlyDetail').subscribe((result) => {
  ...
  openBottomSheet(): void {
    this.filter.filterPage = 'attendance-summary-report'
  ...
  downloadExcel2() {
    ...
    if (can == false) {
      ...
      this.service.post_rqst(this.encryptedData, "DownloadMaster/downloadUserAttendanceMonthlyDetail").subscribe((result) => {
        this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
        if (this.decryptedData['code'] === 1) {
