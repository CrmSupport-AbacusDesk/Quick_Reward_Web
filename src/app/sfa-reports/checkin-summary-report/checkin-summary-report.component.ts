...
@Component({
  selector: 'app-checkin-summary-report',
  templateUrl: './checkin-summary-report.component.html',
  styleUrls: ['./checkin-summary-report.component.scss']
})
export class CheckinSummaryReportComponent implements OnInit {
  ...
  fetchReport(action: any = {}) {
    ...
    this.service.post_rqst(this.encryptedData, 'Reports/userCheckinMonthlyDetail').subscribe((result) => {
  ...
  openBottomSheet(): void {
    this.filter.filterPage = 'checkinSummaryReport'
  ...
  downloadExcel2() {
    ...
    if (can == false) {
      ...
      this.service.post_rqst(this.encryptedData, "DownloadMaster/downloadUserCheckinMonthlyDetail").subscribe((result) => {
        this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
        if (this.decryptedData['code'] === 1) {
