...
@Component({
  ...
  templateUrl: './individual-attendance-report.component.html',
  styleUrls: ['./individual-attendance-report.component.scss']
})
export class IndividualAttendanceReportComponent implements OnInit {
  ...
  fetchReport(action: any = {}) {
    ...
    this.service.post_rqst(this.encryptedData, 'Reports/userAttendanceDetail').subscribe((result) => {
  ...
  openBottomSheet(): void {
    this.filter.filterPage = 'individualAttendance'
  ...
  downloadExcel2() {
    ...
    if (can == false) {
      ...
      this.service.post_rqst(this.encryptedData, "DownloadMaster/downloadUserAttendanceDetail").subscribe((result) => {
        this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
        if (this.decryptedData['code'] === 1) {
