...
@Component({
  selector: 'app-checkin-detail-report',
  templateUrl: './checkin-detail-report.component.html',
  styleUrls: ['./checkin-detail-report.component.scss']
})
export class CheckinDetailReportComponent implements OnInit {
  ...
  fetchReport(action: any = {}) {
    ...
    this.service.post_rqst(this.encryptedData, 'Reports/userCheckinDetail').subscribe((result) => {
  ...
  openBottomSheet(): void {
    this.filter.filterPage = 'checkinDetailReport';
  ...
  downloadExcel2() {
    ...
    if (can == false) { 
      ...
      this.service.post_rqst(this.encryptedData, "DownloadMaster/downloadUserCheckinDetail").subscribe((result) => {
        this.decryptedData = this.service.payLoad ? result : this.cryptoService.decryptData(JSON.stringify(result));
        if (this.decryptedData['code'] === 1) {
