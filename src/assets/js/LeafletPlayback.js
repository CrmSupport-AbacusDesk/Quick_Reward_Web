...
(function (factory) {
  ...
  L.Playback.Clock = L.Class.extend({
    ...
    _tick: function (self) {
      ...
      const timeArray =
        self?._trackController?._tracks[0]?._geoJSON?.properties?.time;
      const distanceArray =
        self?._trackController?._tracks[0]?._geoJSON?.properties?.distance;
      const cursor = self._cursor;

      let km = 0;

      if (timeArray && distanceArray && cursor) {
        let closestTimeIndex = -1;
        let minTimeDifference = Infinity;

        // Find the index of the closest time to the cursor
        for (let i = 0; i < timeArray.length; i++) {
          const timeDifference = Math.abs(timeArray[i] - cursor);
        ...
        // If there's a match, get the km value
        if (closestTimeIndex !== -1) {
          km = distanceArray[closestTimeIndex];
        }
      }
      self._trackController.tock(self._cursor, self._transitionTime);
      self._callbacks(self._cursor, km);
      self._cursor += self._tickLen;
    },

    _callbacks: function (cursor, km) {
    ...
    setCursor: function (ms, km) {
      ...
      this._callbacks(this._cursor, km);
    },
  ...
  L.Playback.DateControl = L.Control.extend({
    ...
    onAdd: function (map) {
      ...
      // setup callback
      playback.addCallback(function (ms, km) {
  ...
  L.Playback = L.Playback.Clock.extend({
    ...
    setData: function (geoJSON) {
      ...
      const isIndex = geoJSON[0].properties.time.findIndex(
        (millisecond) => millisecond === this.getStartTime()
      );
      const km = geoJSON[0].properties.distance[isIndex];
      this.setCursor(this.getStartTime(), km);
    },
