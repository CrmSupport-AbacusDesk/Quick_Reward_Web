...
function B(e, s, t, n) {
  ...
  n === void 0 && (n = {});
  var r = E(e),d = e[2], o = M(r[0]), f = M(r[1]), c = M(t), l = X(s, n.units), p = Math.asin(Math.sin(f) * Math.cos(l) + Math.cos(f) * Math.sin(l) * Math.cos(c)), v = o + Math.atan2(Math.sin(c) * Math.sin(l) * Math.cos(f), Math.cos(l) - Math.sin(f) * Math.sin(p)), g = F(v), k = F(p);

  return O([g, k,d], n.properties);
}
function T(e, s, t) {
...
_.TrackPlayer = class {
  constructor(e, s = {}) {
    ...
    this.track = S(
      t.map(({ lng: a, lat: i ,dateTime: d}) => [a, i,d])
  ...
  playAction(e = !1) {
    ...
    let [t, n,d] = rt(this.track, this.walkedDistance).geometry.coordinates;
    if (this.markerPoint = [n, t,d], this.options.panTo && this.map.panTo(this.markerPoint, {
      animate: !1
    ...
    if (this.options.progress = Math.min(1, this.walkedDistance / s), this.listenedEvents.progressCallback.forEach(
      (r) => {
        ...
        return r(
          ...
          {lat:this.markerPoint[0],lng:this.markerPoint[1],dateTime:this.markerPoint[2]},
          this.trackIndex
        )
      } 
