...
L.Playback.Control = L.Control.extend({
  _html:
    ...
    '              <span id="cursor-time"></span><span id="cursor-km"></span>' +
    "            </a>" +
  ...
  _setup: function () {
    ...
    $("#time-slider").slider({
      ...
      slide: function (event, ui) {
        ...
        const timeArray =
          playback?._trackController?._tracks[0]?._geoJSON?.properties?.time;
        const distanceArray =
          playback?._trackController?._tracks[0]?._geoJSON?.properties
            ?.distance;
        const cursor = ui.value;
        ...
        $("#cursor-km").val(km?.toString());
        $("#cursor-km").html(" (" + km + " KM )");
